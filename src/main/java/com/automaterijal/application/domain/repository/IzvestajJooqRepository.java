...
public class IzvestajJooqRepository {
    ...
    public Page<Komentar> pronadjiSveKomentarePoParametrima(Partner partner, String trazenaRec, Timestamp vremeOd, Timestamp vremeDo, Integer komercijalista, Pageable pageable) {
        ...
        List<Komentar> komentari = select.fetch()
                .stream()
                .map(komentarRecord -> napraviKomentar(komentarRecord))
        ...
        return new PageImpl<>(retVal, PageRequest.of(pageable.getPageNumber(), pageable.getPageSize()), komentari.size());
    }
