...
public class ProizvodjacService {
   ...
   */
  public void popuniProizvodjace(List<RobaDto> robaDtos, MagacinDto magacinDto,
    ...
    if (parametri.getPodgrupaZaPretragu() == null && parametri.getTrazenaRec() == null) {
      ...
      Set<String> proizKljuc = robaDtos.stream().map(RobaDto::getProizvodjac)
          .map(Proizvodjac::getProid).collect(Collectors.toSet());
    ...
    if (parametri.getProizvodjac() != null) {
      boolean trazeniProizvodjacPostoji = proizvodjaci.stream()
          .anyMatch(proizvodjac -> proizvodjac.getProid().equals(parametri.getProizvodjac()));
      if (!trazeniProizvodjacPostoji) {
        proizvodjacRepository.findById(parametri.getProizvodjac())
  ...
  private void getPopuniProizvodjaceURobi(List<RobaDto> robaDtos, List<Proizvodjac> proizvodjaci) {
    for (RobaDto roba : robaDtos) {
      proizvodjaci.stream()
          ...
          .findFirst().ifPresent(roba::setProizvodjac);
    }
  }
  ...
  private List<Proizvodjac> pronadjiSve() {
    return proizvodjacRepository.findAllByOrderByNazivAsc()
        .stream()
  ...
  public List<Proizvodjac> proizvodjaciKateogrija(List<PodGrupa> podgrupe) {
    Set<String> filterRoba = robaService.pronadjiSvuRobuPoPodGrupiIdListaSvaStanja(
            podgrupe.stream().map(PodGrupa::getPodGrupaId).collect(Collectors.toList()))
        .stream()
        .map(robaEnitet -> robaEnitet.getProizvodjac().getProid())
        .collect(Collectors.toSet());
    return proizvodjacRepository.findByProidIn(filterRoba)
        .stream()
